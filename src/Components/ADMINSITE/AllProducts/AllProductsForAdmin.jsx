...
import axios from "axios";
import Swal from "sweetalert2";
import {Link} from "react-router"; // CORRECTED: Use 'react-router-dom' for web
import {AuthContext} from "../../ContextFiles/AuthContext";
import {useQuery, useQueryClient} from "@tanstack/react-query";
import {motion, AnimatePresence} from "framer-motion";
import {
...
const ProductRow = ({product, index, onStatusChange, onDelete, isLast, totalItems}) => {
  const [menuOpen, setMenuOpen] = useState(false);
  ...
  const handleStatusChange = async (id, status) => {
    ...
    if (status === "rejected") {
      const { value: rejectionReason, isConfirmed } = await Swal.fire({
        ...
        preConfirm: (reason) => {
          if (!reason) {
            Swal.showValidationMessage("A reason is required to reject a product.");
          }
          return reason;
        },
      });
      ...
      if (isConfirmed && rejectionReason) {
        try {
          await axios.patch(
            `${BASE_URL}/allProduct/${id}`,
            { status: "rejected", rejectionReason }, // Send reason with the status
            { headers: { Authorization: `Bearer ${accessToken}` } }
          );
          queryClient.invalidateQueries({ queryKey: ["allProductsAdmin"] });
