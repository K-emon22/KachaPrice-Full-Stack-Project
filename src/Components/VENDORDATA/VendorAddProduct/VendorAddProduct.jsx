...
const VendorAddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const product = {
      ...
      vendorEmail: user?.email,
      vendorImage:
        user?.photoURL || "https://i.ibb.co/TDfc7yYp/images-8.png",
    };
    ...
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        toast.success("Product added successfully!");
        toast.info("Awaiting admin approval.");
        setFormData({
          name: "",
          price: "",
          market: "",
  ...
  return (
    <div className="flex items-center justify-center p-4 pt-0 mt-16">
      <motion.div
        className="max-w-3xl w-full bg-white p-8 rounded-2xl shadow-lg"
        variants={containerVariants}
        initial="hidden"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <motion.div className="relative" variants={itemVariants}>
            ...
            <textarea
              ...
              placeholder="Market Description (e.g., specialty, location, timing)"
              className="w-full pl-10 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-all"
              rows="3"
