...
//   return (
//     <div className="max-w-2xl mx-auto p-4 pt-0 bg-white shadow rounded-lg mt-16">
...
import { FaBox, FaDollarSign, FaStore, FaImage, FaAlignLeft, FaSave } from "react-icons/fa";
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (res.data.modifiedCount > 0) {
        ...
        Swal.fire("No Changes", "You haven't made any changes to the product.", "info");
      }
    } catch (error) {
      console.error("Update error:", error);
      Swal.fire("Error", "Failed to update product.", "error");
    ...
    <div className=" pt-0 mt-16 flex items-center justify-center p-4">
      {loading ? (
        ...
        <motion.div
            ...
            animate="visible"
        >
            <motion.div className="text-center mb-8" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1 } }}>
                <h2 className="text-3xl font-bold text-gray-800">Edit Product Listing</h2>
                <p className="text-gray-500 mt-2">Update the details for your product below.</p>
            </motion.div>

            <form onSubmit={handleUpdate} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                    <FormInput icon={<FaBox />} name="name" placeholder="Product Name" value={formData.name} onChange={handleChange} />
                    <FormInput icon={<FaDollarSign />} name="price" placeholder="Current Price" type="number" value={formData.price} onChange={handleChange} />
                ...
                <motion.div className="relative" variants={{ hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0 } }}>
